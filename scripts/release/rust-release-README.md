...
令牌与私钥不打印、不写文件、不传给 Wrangler 或 esbuild/worker-build。网络错误体、上传 URL 和 Wrangler stdout/stderr 不输出；只报告安全阶段与 HTTP 状态。公共密钥文件不包含秘密。
Tokens/private keys are never printed, written to files or passed to Wrangler/esbuild/worker-build. Network error bodies, upload URLs and Wrangler stdout/stderr are suppressed; only safe phase/status information is reported. Public key files contain no secrets.
...
上传 SHA-256 来自真实字节；Base64 MD5 只用于传输校验。上传通过同源 Worker 原生 R2 PUT，不需要 S3 密钥。客户端只向精确的 `/v1/deployments/{deployment_id}/artifact-uploads/{upload_id}` 路径主动附加 machine bearer，拒绝其他 origin、query、userinfo、fragment、非规范编码与重定向。会话只能指定 content-type、content-length、content-md5、if-none-match 四个匹配产物的 headers，必须包含 `If-None-Match: *`；任何身份 header 注入均失败。每件产物在读取前限制为 64 MiB，source map 为 8 MiB。PUT 412 只允许继续服务端 digest commit 验证。同 attempt 重试复用幂等 key；会话过期后才递增 `release_attempt`。ready 失败不执行云版本上传；上传失败/receipt 缺失、多个或非法不切换流量。发布不查询可能竞争的 latest version。
SHA-256 hashes actual bytes; Base64 MD5 is only a transport check. Uploads use same-origin Worker-native R2 PUT without S3 credentials. The client adds its machine bearer only to the exact `/v1/deployments/{deployment_id}/artifact-uploads/{upload_id}` endpoint, rejecting other origins, queries, userinfo, fragments, noncanonical encodings and redirects. Sessions may provide only artifact-matching content-type/content-length/content-md5/if-none-match headers; `If-None-Match: *` is mandatory and identity-header injection fails closed. Each artifact is limited to 64 MiB before reading, with an 8 MiB source-map limit. PUT 412 only permits server-side digest commit verification. Reuse idempotency keys within an attempt; increment the attempt after session expiry. Failed readiness prevents cloud version upload; failed uploads or missing/ambiguous/invalid receipts prevent traffic cut-over. Never query a racing latest version.
...
发布成功仅输出非秘密 JSON receipt，包含 `deployment_id`、精确 `version_id`、`state=ready` 与 `activation=pending`；这不代表管理员激活已经完成。
Successful publication emits a non-secret JSON receipt containing `deployment_id`, exact `version_id`, `state=ready` and `activation=pending`; it does not claim administrator activation.
