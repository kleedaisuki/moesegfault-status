...
本文定义 `moesegfault-status` 的权威领域数据库契约。它是 [`status-design.md`](./status-design.md) 与 [`observability-standard.md`](./observability-standard.md) 的实现伴随文档；列、约束与索引的最终事实来源是 [`../migrations`](../migrations) 中按编号顺序应用的完整迁移链，不能只读取初始 migration。

This document defines the authoritative domain database contract for `moesegfault-status`. It accompanies [`status-design.md`](./status-design.md) and [`observability-standard.md`](./observability-standard.md); the final source of truth for columns, constraints, and indexes is the complete numbered chain in [`../migrations`](../migrations), not the initial migration alone.
...
| 策略          | `evaluation_policies`, `service_diagnostic_policies`, `data_retention_policies`, `service_retention_policies`                                                                               | 精确 assignment 与 `(policy_id, revision)`                                              | 策略修订不可更新/删除；selector 按固定特异度选择，不得靠“最大 revision”猜测    |
...
永久 `issue_telemetry_references` 可以选择性指向 occurrence。0005 将该外键改为 `ON DELETE SET NULL`，并把不可变 trigger 收窄为只允许数据库执行 `occurrence_id: non-null → NULL`；因此未被 Incident 固定的摘要到期时，Issue→TelemetryReference 的永久来源关系仍保留。应用不能借此改绑到另一个 occurrence，也不能删除 evidence link。`incident_occurrences` 仍使用 RESTRICT/阻断 trigger，Incident 引用绝不会被 retention detach。
...
同一 artifact digest 可以被新 deployment ID 重新部署或回滚；`(service_name, environment, artifact_digest)` 只是普通查询索引，**不是唯一约束**。`artifact_upload_sessions.object_key` 与 `deployment_artifacts.object_key` 同样只是非唯一查询索引：多个会话可以续签同一不可变对象，多个 Deployment 可以各自保留指向同一内容寻址对象的来源关系。部署内 artifact 的稳定身份是 `UNIQUE(deployment_id, kind, file_name)`；把 digest 放进唯一键反而会允许同一声明位置被悄悄换内容。
...
0004 为 upload session 增加 `content_md5`，它是 16-byte MD5 的 24 字符 Base64（结尾 `==`），用于签名上传时的 R2 `Content-MD5` 传输完整性检查；权威 provenance 仍使用 SHA-256，MD5 不是内容身份。历史会话无法诚实回填，因此迁移保留 `NULL`；新 session 的 insert trigger 强制非空且格式合法，旧 outstanding session 必须续建后才能 commit。迁移前已经验证并提交的 artifact 不做破坏性追溯失效。

推荐不可变 R2 key：

```text
observability/manifests/<deployment-id>.json
observability/artifacts/sha256/<hex>
```

应用在提交 artifact 前仍须对 R2 `HEAD` 结果核对大小与可信 checksum metadata；SQL 无法证明远端对象内容。共享对象的 R2 metadata 不得存放唯一 `deployment_id`/Git commit 并把它冒充全部来源；每个 Deployment 的精确 provenance 位于 D1 关系行和不可变 Manifest 中。

### 4.4 Diagnostic policy selector

`service_diagnostic_policies` 的 `assignment_id` 是稳定 UUIDv7。selector 恰有三种：

| `selector_kind`   | 必需字段 / Required fields        | 用途 / Purpose                                    |
| ----------------- | --------------------------------- | ------------------------------------------------- |
| `monitor`         | `monitor_id`                      | 可信 Queue envelope 声明来源 monitor 时的精确覆盖 |
| `service_kind`    | `service_name`, `diagnostic_kind` | 某服务的一类 Diagnostic                           |
| `service_default` | `service_name`                    | 没有更具体匹配时的默认值                          |

三个部分唯一索引保证每个特异度最多一个答案。consumer 必须按 `monitor → service_kind → service_default` 排序并取第一行；不得按 assignment 时间或 policy revision 取“最新”。`monitor_id` 只接受内部 Queue envelope 的可信 `origin`，不能接受公网 DiagnosticEvent 自报。0003 会把 0001 的服务级旧行无损迁移为 `service_default` 并赋予稳定 assignment ID。

### 4.5 Incident 是日志，不是可覆写文档
...
R2、Queue、Analytics Engine 和 telemetry backend 不加入 D1 transaction。D1 同时保存领域变化与事务发件箱（**transactional outbox**），异步 dispatcher 至少一次发送；接收者仍须按 `outbox_id` 幂等。该取舍符合生产系统避免伪两阶段提交的常见实践，也与分布式系统研究中“本地事务保存意图、跨边界用可重试消息协调”的方向一致；参见 Pat Helland 的 CIDR 论文 [_Life beyond Distributed Transactions_](https://www.cidrdb.org/cidr2007/papers/cidr07p15.pdf)。
...
- 0002 对既有 session/artifact 的无损重建、会话续签和跨 Deployment 对象复用；
- 0003 对旧 policy assignment 的无损升级，以及 monitor/service-kind/default 固定优先级；
- 0004 保留旧 session/已提交 artifact、新 session 强制 Content-MD5、旧 outstanding session 必须续建；
- 0005 允许 evidence-linked 但未被 Incident 固定的 occurrence 到 摘要清理，同时永久保留 evidence provenance；
