...
| public                  | 六个公开查询入口、分页、状态新鲜度、错误响应 / six public reads, pagination, freshness, errors                                        | Incident 列表/详情、游标和分页已迁移并通过 workerd；状态/服务/维护窗口待迁移 / Incident list/detail and pagination verified in workerd; status/services/maintenance pending          |
