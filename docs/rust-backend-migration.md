...
管理认证改为 Rust `admin_auth`：预配置密码登录/退出，无 Access、setup、注册或改密 UI。Worker secret `ADMIN_PASSWORD_RECORD` 为 PBKDF2-SHA256、100,000 次迭代、16 字节盐和 32 字节哈希。D1 0009 只保存会话摘要/记录指纹和全局 30 次/10 分钟登录预算；会话 12 小时，Secure/HttpOnly/SameSite=Strict。密码记录轮换必须将新版本切换到 100% 流量，才能全部拒绝旧记录对应会话。 / Rust admin_auth uses a preconfigured owner password, not Access or account-management UI. The secret uses PBKDF2-SHA256/100,000 iterations/16-byte salt/32-byte hash. D1 stores only session digests/fingerprints and a global login budget; secure sessions last 12 hours. A complete version cutover is required for complete old-session revocation.

此参数仅服务于本机 CLI 用密码学安全随机数生成的 24 字节（192-bit，32 字符 base64url）管理员凭据，并适配生产 Workers Web Crypto 的 PBKDF2 迭代上限；不支持人工选择的低熵密码，也不是通用人类密码存储建议。CLI 没有密码输入参数；上传记录本身无法证明原始密码的熵，必须保持受控生成流程。 / This parameter serves only the locally generated 24-byte (192-bit, 32-character base64url) cryptographically random administrator secret and fits production Workers Web Crypto PBKDF2 limits. Human-chosen low-entropy passwords are unsupported; this is not generic human-password storage guidance. The CLI has no password-input argument; a verifier alone cannot prove password entropy, so preserve controlled generation.
...
Rust status bootstrap、Ops gateway/Static Assets、两个 Custom Domain 和管理员 secret 安装已实际执行。公共 JWKS HTTPS 200 且公钥一致；Ops HTML HTTPS 200 且构建 SHA-256 一致。status 查询与 Ops session 仍 503，未宣称云端登录或完整生产 ready。初始 Cron 配置因缺账户 workers.dev subdomain 报 10063，但脚本和四个 producer 已上传；后续不含 Cron 的域名配置成功。账户子域已初始化，10063 前置解除但默认 URL/preview 仍关闭；后续实际启用结果以部署记录为准。上传已改原生 R2 认证 PUT，无需 S3 凭据；通知默认关闭。 / Bootstrap, static UI, domains and the administrator secret are installed. JWKS and HTML match their source/build and return 200; status/session remain 503. Initial Cron configuration failed with 10063 after successful script/producer upload; domain configuration later succeeded without Cron. The account subdomain now resolves the Cron prerequisite while default URLs/previews remain disabled. Native authenticated R2 PUT no longer requires S3 credentials; notifications default to disabled. A complete ready release still requires evidence. See the operations runbook for the exact evidence boundary.

## 当前部署选择 / Current deployment choices

原生 R2 binding 的机器 JWT 认证 PUT 替代 S3 预签名传输：64 MiB artifact、8 MiB source map、600 秒 session、四个精确请求头、同源且不重定向，commit/ready 不变。NOTIFICATIONS_ENABLED 默认 false；外部 outbox pending 且 attempts 不增，启用前不部署通知 consumer；TELEMETRY_AUTH_JSON 可选。本机已原子初始化三个真实内部服务身份、3 条 audit 和 3 条 catalog.changed outbox，monitor/component 均为 0；GitHub Actions 实际发布 status/ops/probe 后，再由 owner 在 ready 与 smoke 通过后激活。 / Native R2 transport preserves bounded authenticated uploads and commit/readiness without S3 credentials. Notifications default off with pending outbox and no retry inflation; telemetry credentials are optional. Real platform identities/audit/outbox are atomically initialized with no monitors/components or fabricated healthy samples; then release through Actions and activate after readiness/smoke evidence.
...
三个 production 构建与本机 46 MiB 原生 R2 上传、DigestStream commit→ready 专项测试已通过；正式 Actions 运行与激活结果仍待最终收口。 / Three production builds and the local 46 MiB streaming-digest upload/readiness test passed; production Actions and activation still require final evidence.
